...
def init_board(nb_rows, nb_cols, value):
    ...
    temp_var = 0 # Start from 0
    
    while temp_var < nb_rows: 
        temp_var += 1 
        y = []
...
def play():
    ...
    #sets no of mines based on difficulty
    if diff == "EASY":
    ...
    #generates two separate helper and game board
    ...
    while count_total(game_board, "?") +\
     count_total(game_board, "⚑") > nb_mines:
        
        user_mines = nb_mines - count_total(game_board, "⚑")
        print("Current board:" + str(user_mines) + " mines remaining")
        print(game_board)

        action = input("Choose 0 to reveal or 1 to flag: ")
